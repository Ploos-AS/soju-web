...
func (a *app) generateCertFP(w http.ResponseWriter, r *http.Request) {
	...
	if user == "" || network == "" || r.FormValue("confirm") != "generate" {
		http.Error(w, "user, network and generate confirmation are required", http.StatusBadRequest)
		return
	}
...
{{end}}</pre>{{end}}<p class="warn">Generating a certificate changes this network to SASL EXTERNAL and replaces the existing SASL credential material.</p><form method="post" action="/security/certfp/generate"><input type="hidden" name="csrf" value="{{.CSRF}}"><input type="hidden" name="user" value="{{.User}}"><input type="hidden" name="network" value="{{.Network}}"><label>Key type<select name="key_type"><option value="ed25519">Ed25519</option><option value="ecdsa">ECDSA</option><option value="rsa">RSA-3072</option></select></label><label>Type <code>generate</code> to confirm<input name="confirm" autocomplete="off" required></label><button>Generate certificate</button></form></section>{{end}}</main></body></html>`
