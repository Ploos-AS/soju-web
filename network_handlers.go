...
func (a *app) networksPage(w http.ResponseWriter, r *http.Request) {
	...
	if username != "" {
		...
		data.Lines = lines
		networks := parseNetworkStatuses(lines)
		data.Networks = makeNetworkRows(username, networks)
		manage := strings.TrimSpace(r.URL.Query().Get("manage"))
		for i := range networks {
			if networks[i].Name == manage {
				selected := networks[i]
...
const networksTemplate = `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Networks · soju-web</title><style>` + baseCSS + ` nav a{color:#fbbf24;margin-right:1rem}table{width:100%;border-collapse:collapse}th,td{text-align:left;padding:.7rem;border-bottom:1px solid #374151}.badge{display:inline-block;padding:.15rem .5rem;border-radius:999px;background:#374151}.badge.ok{background:#064e3b}.badge.bad{background:#7f1d1d}.badge.warn{background:#78350f}.actions a{color:#fbbf24;text-decoration:none;margin-right:.75rem;white-space:nowrap}select{font:inherit;padding:.7rem;border-radius:8px;border:1px solid #4b5563;background:#111827;color:#fff;margin:.4rem 0 1rem;width:100%}</style></head><body><main><header><div><h1>Networks</h1><nav><a href="/">Dashboard</a><a href="/users">Users</a><a href="/networks">Networks</a><a href="/channels">Channels</a><a href="/security">Security</a></nav></div><form method="post" action="/logout"><button type="submit">Sign out</button></form></header><section><h2>Select user</h2><form method="get" action="/networks"><label>soju username<input name="user" value="{{.User}}" required></label><button type="submit">Load networks</button></form></section>{{if .Notice}}<section><p class="ok">{{.Notice}}</p></section>{{end}}{{if .User}}<section><h2>Network status for {{.User}}</h2><p class="muted"><a href="/users">← Users</a> · Admin socket: <code>{{.SocketPath}}</code></p>{{if .Error}}<p class="bad">{{.Error}}</p>{{else if .Networks}}<table><thead><tr><th>Network</th><th>Address</th><th>Status</th><th>Nick</th><th>Channels / detail</th><th>Actions</th></tr></thead><tbody>{{range .Networks}}<tr><td><code>{{.Name}}</code></td><td><code>{{.Address}}</code></td><td>{{if eq .State "connected"}}<span class="badge ok">connected</span>{{else if eq .State "disabled"}}<span class="badge warn">disabled</span>{{else}}<span class="badge bad">{{.State}}</span>{{end}}</td><td>{{if .ConnectedAs}}{{.ConnectedAs}}{{else}}—{{end}}</td><td>{{if .Detail}}{{.Detail}}{{else}}—{{end}}</td><td class="actions"><a href="{{.ManageURL}}">Manage</a><a href="{{.ChannelsURL}}">Channels →</a><a href="{{.SecurityURL}}">Security →</a></td></tr>{{end}}</tbody></table>{{else}}<p class="muted">No networks configured.</p>{{end}}</section><section><h2>Create network</h2><form method="post" action="/networks/create"><input type="hidden" name="csrf" value="{{.CSRF}}"><input type="hidden" name="user" value="{{.User}}"><label>Address<input name="addr" placeholder="irc.libera.chat or ircs://irc.libera.chat:6697" required></label><label>Name<input name="network_name"></label><label>Nick<input name="nick"></label><label>IRC username<input name="irc_username"></label><label>Real name<input name="realname"></label><label>Enabled<select name="enabled"><option value="true">Yes</option><option value="false">No</option></select></label><button type="submit">Create network</button></form></section><section id="manage-network"><h2>{{if .Selected}}Manage {{.Selected.Name}}{{else}}Manage network{{end}}</h2><form method="post" action="/networks/update"><input type="hidden" name="csrf" value="{{.CSRF}}"><input type="hidden" name="user" value="{{.User}}"><label>Current network name<input name="name" value="{{if .Selected}}{{.Selected.Name}}{{end}}" required></label><label>Address<input name="addr" value="{{if .Selected}}{{.Selected.Address}}{{end}}"></label><label>New name (optional)<input name="network_name"></label><label>Nick (optional)<input name="nick"></label><label>IRC username (optional)<input name="irc_username"></label><label>Real name (optional)<input name="realname"></label><label>Enabled<select name="enabled"><option value=""{{if not .Selected}} selected{{end}}>Unchanged</option><option value="true"{{if and .Selected (ne .Selected.State "disabled")}} selected{{end}}>Yes</option><option value="false"{{if and .Selected (eq .Selected.State "disabled")}} selected{{end}}>No</option></select></label><button type="submit">Update network</button></form><h3>Delete network</h3><form method="post" action="/networks/delete"><input type="hidden" name="csrf" value="{{.CSRF}}"><input type="hidden" name="user" value="{{.User}}"><label>Network name<input name="name" value="{{if .Selected}}{{.Selected.Name}}{{end}}" required></label><label>Type <code>delete</code> to confirm<input name="confirm" required></label><button type="submit">Delete network</button></form></section>{{end}}</main></body></html>`
