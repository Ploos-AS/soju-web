...
func (a *app) createNetwork(w http.ResponseWriter, r *http.Request) {
	if !a.prepareAdminPost(w, r) { return }
	username := strings.TrimSpace(r.FormValue("user")); addr := strings.TrimSpace(r.FormValue("addr"))
	if username == "" || !validNetworkAddress(addr) { http.Error(w, "user and a valid network address are required", http.StatusBadRequest); return }
	words := []string{"user", "run", username, "network", "create", "-addr", addr}; words = appendNetworkOptions(words, r)
	ctx, cancel := context.WithTimeout(r.Context(), 6*time.Second); defer cancel()
	if _, err := a.adminClient().Run(ctx, words...); err != nil { http.Error(w, "soju: "+err.Error(), http.StatusBadGateway); return }
...
func (a *app) updateNetwork(w http.ResponseWriter, r *http.Request) {
	if !a.prepareAdminPost(w, r) { return }
	username := strings.TrimSpace(r.FormValue("user")); name := strings.TrimSpace(r.FormValue("name"))
	if username == "" || name == "" { http.Error(w, "user and network name are required", http.StatusBadRequest); return }
	words := []string{"user", "run", username, "network", "update", name}
	if addr := strings.TrimSpace(r.FormValue("addr")); addr != "" { if !validNetworkAddress(addr) { http.Error(w, "invalid network address", http.StatusBadRequest); return }; words = append(words, "-addr", addr) }
...
func (a *app) deleteNetwork(w http.ResponseWriter, r *http.Request) {
	if !a.prepareAdminPost(w, r) { return }
	username := strings.TrimSpace(r.FormValue("user")); name := strings.TrimSpace(r.FormValue("name"))
	if username == "" || name == "" || r.FormValue("confirm") != "delete" { http.Error(w, "user, network name and delete confirmation are required", http.StatusBadRequest); return }
	ctx, cancel := context.WithTimeout(r.Context(), 6*time.Second); defer cancel()
	if _, err := a.adminClient().Run(ctx, "user", "run", username, "network", "delete", name); err != nil { http.Error(w, "soju: "+err.Error(), http.StatusBadGateway); return }
...
func appendNetworkOptions(words []string, r *http.Request) []string {
	for _, field := range []struct{ form, flag string }{{"network_name", "-name"}, {"irc_username", "-username"}, {"nick", "-nick"}, {"realname", "-realname"}} { if v := strings.TrimSpace(r.FormValue(field.form)); v != "" { words = append(words, field.flag, v) } }
	if v := strings.TrimSpace(r.FormValue("enabled")); v != "" { words = append(words, "-enabled", fmt.Sprint(boolValue(v))) }
	return words
}
...
func validNetworkAddress(addr string) bool {
	...
	if !strings.Contains(addr, "://") { return !strings.ContainsAny(addr, " \t") }
	u, err := url.Parse(addr); return err == nil && u.Host != "" && (u.Scheme == "ircs" || u.Scheme == "irc+insecure")
}

func networkNoticeText(v string) string { switch v { case "created": return "Network created."; case "updated": return "Network updated."; case "deleted": return "Network deleted."; default: return "" } }
