...
func (a *app) channelsPage(w http.ResponseWriter, r *http.Request) {
	...
	if username != "" && network != "" {
		...
		data.Lines = lines
		channels := parseChannelStatuses(lines)
		data.Channels = makeChannelRows(username, network, channels)
		manage := strings.TrimSpace(r.URL.Query().Get("manage"))
		for i := range channels {
			if channels[i].Name == manage {
				selected := channels[i]
